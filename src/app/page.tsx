...
import { Menu, Maximize, Minimize, PlayCircle, Download, Globe, LogOut, X, User as UserIcon, ChevronRight, Zap, Users, Trophy, Target } from 'lucide-react';
...
export default function Home() {
  ...
  const handleJoin = () => {
    if (roomCode.trim()) {
      ...
      router.push(`/player/${roomCode.trim()}/login?nickname=${encodeURIComponent(nick)}`);
    }
  };
  ...
  return (
    <div className="bg-[#0b101a] text-white min-h-screen relative overflow-hidden font-body selection:bg-[#2d6af2] selection:text-white flex flex-col">
      ...
      {user && (
        <div className="fixed top-3 right-4 md:top-3 md:right-6 z-[100]" ref={dropdownRef}>
          <button
              ...
              >
                ...
                <div className="p-6 bg-gradient-to-br from-white/[0.05] to-transparent border-b border-white/5">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] text-[#2d6af2] font-display tracking-[0.2em] uppercase mb-0.5">Verified Racer</p>
                      <p className="text-white text-lg font-bold truncate tracking-tight">{user.username}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 flex flex-col gap-1">
                  ...
                  >
                    <div className="p-2 rounded-xl bg-gray-500/10 group-hover:bg-[#2d6af2]/20 transition-colors">
                      {isFullscreen ? <Minimize className="w-4 h-4 text-[#2d6af2]" /> : <Maximize className="w-4 h-4 text-[#2d6af2]" />}
                    </div>
                    <span className="text-sm font-medium tracking-wide">Fullscreen</span>
                  </button>

                  <button
                    onClick={() => { setShowHowToPlay(true); setIsDropdownOpen(false); }}
                    className="flex items-center gap-4 w-full px-4 py-3.5 rounded-2xl hover:bg-white/5 text-gray-400 hover:text-white transition-all group"
                  >
                    <div className="p-2 rounded-xl bg-gray-500/10 group-hover:bg-[#2d6af2]/20 transition-colors">
                      <PlayCircle className="w-4 h-4 text-[#2d6af2]" />
                    </div>
                    <span className="text-sm font-medium tracking-wide">How to Play</span>
                  </button>
                  ...
                  <button className="flex items-center gap-4 w-full px-4 py-3.5 rounded-2xl hover:bg-white/5 text-gray-400 hover:text-white transition-all group">
                    ...
                    <span className="text-sm font-medium tracking-wide">Language (EN)</span>
                  </button>
                </div>
                ...
                <div className="p-3 bg-black/20 border-t border-white/5">
                  ...
                  >
                    ...
                    <span className="text-sm tracking-[0.1em] uppercase">Disconnect</span>
                  </button>
                </div>
              </motion.div>
            )}
            ...
            >
              ...
              <div className="p-6 pb-4 flex items-center justify-between border-b border-white/5">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-display uppercase tracking-wider text-white drop-shadow-[0_0_10px_rgba(45,106,242,0.5)]">How to Play</h2>
                </div>
                <button
                  onClick={() => setShowHowToPlay(false)}
              ...
              <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                {[
                  { icon: <Zap className="w-5 h-5" />, title: "Host or Join", desc: "Create a room as Host to select a quiz and invite players, or enter a Room Code to Join as a player.", color: "text-[#00ff9d]", bg: "bg-[#00ff9d]/10 border-[#00ff9d]/20" },
                  { icon: <Target className="w-5 h-5" />, title: "Select a Quiz", desc: "As a Host, browse and pick from available quizzes. Configure the number of questions, duration, and difficulty.", color: "text-[#2d6af2]", bg: "bg-[#2d6af2]/10 border-[#2d6af2]/20" },
                  { icon: <Users className="w-5 h-5" />, title: "Wait for Players", desc: "Share your Room Code with friends. Once everyone has joined the lobby, the Host starts the game.", color: "text-purple-400", bg: "bg-purple-400/10 border-purple-400/20" },
                  { icon: <Trophy className="w-5 h-5" />, title: "Answer & Race!", desc: "Answer questions as fast as possible! Points are awarded based on speed and accuracy. The fastest correct answer wins!", color: "text-yellow-400", bg: "bg-yellow-400/10 border-yellow-400/20" },
                ].map((step, i) => (
                  <motion.div
                    key={i}
                  ...
                  >
                    <div className={`flex-shrink-0 p-2 rounded-xl ${step.bg} ${step.color}`}>
                      {step.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] font-display tracking-widest text-gray-500 uppercase">Step {i + 1}</span>
                      </div>
                      <h3 className={`font-display text-sm uppercase tracking-wider mb-1 ${step.color}`}>{step.title}</h3>
